...
첫 배포 `a28c660`은 GitHub Actions 전체 회귀 1,921개 통과·1개 선택 검사 제외 후 완료됐다. 공개 파일 18개의 바이트와 실제 브라우저 조작까지 확인했다.

이후 예약 실행에서 배포 확인기가 새 연구 다운로드 3개를 예상 파일 목록에서 누락한 문제가 발견됐다. `verify_public_readback`도 배포 생성기와 같은 payload-bound 다운로드 계약을 사용하도록 연결했다. 공개 manifest의 파일 목록·크기·해시와 검토 payload의 정확한 내용 일치를 모두 유지한다. 정상 배포본이 불필요한 재배포 경로로 들어가지 않는지도 실제 배포 생성기로 만든 패키지로 검사한다.

동시에 기존 승인 문서는 Excel 입력 지원을 위해 추가한 `openpyxl`·`et-xmlfile` 이전의 lock 해시를 보유하고 있었다. Python·OS·모델 패키지는 모두 동일했다. 사용자가 승인한 배포본의 정확한 lock과 설치 버전을 대조한 뒤 기존 승인 범위·확인일·만료일·0600 권한을 보존해 runtime fingerprint만 결속했다. 원본 문서는 비공개 로컬 경로에 백업했다. 환경 불일치는 변경이 해결될 때까지 대기하는 오류로 분류해 반복 재시도도 막는다.

실제 공개 사이트의 자동화 readback은 통과했다. 관련 회귀 136개와 환경 불일치 오류 분류 3개도 통과했다. 재개 검증과 원본 보존 증거는 `build/forecast-audit-improvements/resume-20260907/`에 있다.
